...
local hyper={"alt","ctrl","command"}
local desktop_hyper={"alt","ctrl","command","shift"}
local space_hyper={"ctrl","alt"}
local move_space_hyper={"shift","ctrl","alt"}
...
local function moveWindowOneSpace(direction)
  ...
  if win:application():title() == 'Google Chrome' or win:application():title()=='iTerm2'  then
      local target=nil
...
eventtapMiddleMouseDown = hs.eventtap.new({ hs.eventtap.event.types.middleMouseDown }, function(event)
	hs.alert.show("middleMouseDown")
	hs.eventtap.event.newKeyEvent("",hs.keycodes.map.padenter, true):post()
	return false
end)
